...
MizFlexLexer::MizFlexLexer(std::istream *in,
    ...
    : yyMizFlexLexer(in), symbol_table_(symbol_table),
      token_array_(std::make_shared<TokenArray>()), line_number_(1),
      column_number_(1), is_in_environ_section_(false),
      is_in_vocabulary_section_(false) {}

size_t MizFlexLexer::ScanSymbol() {
  Symbol *symbol = symbol_table_->QueryLongestMatchSymbol(yytext);
  if (symbol) {
    Token *token = new Token(line_number_, column_number_, TOKEN_TYPE::SYMBOL,
                             nullptr, symbol);
