...
Token::Token(size_t line_number, size_t column_number, TOKEN_TYPE type,
    ...
    : line_number_(line_number), column_number_(column_number), type_(type),
      text_(nullptr) {
  if (text) {
    text_ = strdup(text);
    if (!text_) {
...
Token::Token(const Token &rhs)
    : line_number_(rhs.line_number_), column_number_(rhs.column_number_),
      type_(rhs.type_), text_(nullptr) {
